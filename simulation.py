# simulation.py
# Version adaptée pour Pyodide, proche de ton script original mais limitée à jetlet niveau 0
# Retourne Ux_fields, Uy_fields (par frame) si compute_field True.
...
# Bruit optionnel (désactivé par défaut pour rester fidèle à ton souhait)
K_noise = 0
noise_amp = 0.25
L_domain = 10.0
...
def simulate(steps=None, dt=None, n_image_local=None, grid_nx_field=None, grid_ny_field=None,
    ...
    for i in range(n_image_local):
        # store positions + compute per-particle velocities
        for a in range(N):
    ...
    if compute_field and grid_pts.shape[0] > 0:
        for i in range(n_image_local):
            uv = u_au_point_vectorized(grid_pts, qs_hist[i], np.array([state[2*N+2*j:2*N+2*j+2] for j in range(N)]) if False else np.zeros((0,2)), sigma=sigma)
            # NOTE: u_au_point_vectorized requires ps current; we should use ps history per frame
            # we have ps not stored separately here so re-evaluate ps from the state snapshots stored earlier is needed.
            # simpler: recompute using qs_hist and u_selfs? For correctness, recompute ps_hist from initial + integration (but we didn't store ps_hist to save memory).
            # To keep behavior correct: we re-run to compute ps_hist quickly (N small). We'll recompute ps_hist by re-simulating briefly.
        # To avoid complexity and given N small, let's recompute ps_hist properly by rerunning the sim but storing ps too.

    # --- simpler and safe approach: rerun storing ps per frame (we do it robustly below) ---

def simulate(steps=None, dt=None, n_image_local=None, grid_nx_field=None, grid_ny_field=None,
             grid_extent_local=None, rng_seed=None, compute_field=False):
    # (Implementation duplicate but with ps_hist stored — this simplifies Ux/Uy computation)
    if n_image_local is None:
        n_image_local = n_image
    if steps is None:
        steps = n_image_local
    ...
    # compute Ux, Uy per frame if requested
    if compute_field and grid_pts.shape[0] > 0:
        for i in range(n_image_local):
    ...
    # prepare output: convert to lists
    out = {
        ...
        'Ux_fields': [frame.tolist() for frame in Ux_fields],
        'Uy_fields': [frame.tolist() for frame in Uy_fields],
        'grid_x': xs.tolist(),
        'grid_y': ys.tolist(),
        'N': int(N),
